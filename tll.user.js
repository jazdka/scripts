  ...
  (() => {
    ...
    async function stealSomeArtFlow() {
      // 1) coords come from Template Library form
      let c = getTLCoordsFromForm();
  
      // 2) if missing -> offer to import from map selection
      if (!c) {
        const want = confirm(
          "Coords are not set in the Template Library.\n\n" +
            "Click OK to use the last pixel you clicked on the map (from BlueMarble).\n" +
            "Click Cancel to abort.",
        );
        if (!want) return;
  
        const fromMap = parseLastClickedFromBmSpan();
        if (!fromMap) {
          alert(
            "No map pixel found.\n\n" +
              "1) Click any pixel on the map first (so BlueMarble shows coords)\n" +
              "2) Then try “Steal some art” again.",
          );
          return;
        }
  
        // Fill the TL form too (so next time it’s already set)
        setInputValue(qs("#bm-lib-tlx"), fromMap.tlx);
        setInputValue(qs("#bm-lib-tly"), fromMap.tly);
        setInputValue(qs("#bm-lib-px"), fromMap.px);
        setInputValue(qs("#bm-lib-py"), fromMap.py);
  
        c = fromMap;
      }
  
      const wStr = prompt("Steal width (pixels)?", "128");
      if (wStr === null) return;
      const hStr = prompt("Steal height (pixels)?", "128");
      if (hStr === null) return;
      ...
      if (
        ...
        alert("Width/height must be valid numbers (1..4096).");
        return;
      }
      ...
      // Filename based on top-left coordinate
      const name =
        `wplace_steal_TL_${String(c.tlx).padStart(4, "0")},${String(c.tly).padStart(4, "0")},` +
        `${String(c.px).padStart(3, "0")},${String(c.py).padStart(3, "0")}` +
        `_${width}x${height}_${iso}.png`;
  
      try {
        // TOP-LEFT anchored capture now:
        ...
        console.error(e);
        alert(
          "Failed to steal pixels.\n\n" +
            "If you see “unreachable” tile errors, the tile server might be down / rate-limiting.\n" +
            `Details: ${e?.message || e}`,
        );
      }
    }
