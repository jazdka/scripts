...
(() => {
  ...
  function renderLibraryUI() {
    ...
    panel.id = "bm-lib";
    // Build panel via DOM (avoids template-literal corruption in Tampermonkey)
    ...
    function mkRow() { return mkEl("div", {"class":"row"}); }

    var bar = mkEl("div", {"class":"bar","id":"bm-lib-bar"});
    bar.appendChild(mkEl("div", {"class":"title"}, "Template Library"));
    bar.appendChild(mkEl("span", {"id":"bm-lib-verbadge","class":"bm-lib-ver-badge unknown"}, "BM ?"));
    bar.appendChild(mkEl("button", {"id":"bm-lib-min","type":"button","title":"Minimize"}, "-"));
    panel.appendChild(bar);

    var body = mkEl("div", {"class":"body"});
    var rSel = mkRow(); rSel.appendChild(mkEl("select",{"id":"bm-lib-select","size":"8"})); body.appendChild(rSel);

    var r1 = mkRow();
    r1.appendChild(mkEl("button",{"class":"action","id":"bm-lib-steal","type":"button","title":"Download pixels from map"},"Steal some art"));
    r1.appendChild(mkEl("button",{"class":"action","id":"bm-lib-loadcreate","type":"button","title":"Load then click BM Create"},"Load + Create"));
    body.appendChild(r1);

    var r2 = mkRow();
    var upW = mkEl("div",{"style":"flex:1"}); upW.appendChild(mkEl("button",{"class":"small","id":"bm-lib-moveup","type":"button","title":"Move up"},"^")); r2.appendChild(upW);
    var dnW = mkEl("div",{"style":"flex:1"}); dnW.appendChild(mkEl("button",{"class":"small","id":"bm-lib-movedown","type":"button","title":"Move down"},"v")); r2.appendChild(dnW);
    var dlW = mkEl("div",{"style":"flex:2"}); dlW.appendChild(mkEl("button",{"class":"action","id":"bm-lib-deletecurrent","type":"button"},"Delete current")); r2.appendChild(dlW);
    body.appendChild(r2);

    var r3 = mkRow();
    r3.appendChild(mkEl("button",{"class":"action","id":"bm-lib-importmap","type":"button","title":"Requires pixel selected on map"},"Import from map"));
    r3.appendChild(mkEl("button",{"class":"action","id":"bm-lib-importbm","type":"button","title":"Copies coords from BM"},"Import from BM"));
    body.appendChild(r3);

    var r4 = mkRow(); r4.appendChild(mkEl("input",{"id":"bm-lib-name","type":"text","placeholder":"Name (required, unique for NEW)"})); body.appendChild(r4);

    var r5 = mkRow();
    r5.appendChild(mkEl("input",{"id":"bm-lib-tlx","class":"mono","type":"number","placeholder":"Tl X","min":"0","max":"2047","step":"1"}));
    r5.appendChild(mkEl("input",{"id":"bm-lib-tly","class":"mono","type":"number","placeholder":"Tl Y","min":"0","max":"2047","step":"1"}));
    body.appendChild(r5);

    var r6 = mkRow();
    r6.appendChild(mkEl("input",{"id":"bm-lib-px","class":"mono","type":"number","placeholder":"Px X","min":"0","max":"2047","step":"1"}));
    r6.appendChild(mkEl("input",{"id":"bm-lib-py","class":"mono","type":"number","placeholder":"Px Y","min":"0","max":"2047","step":"1"}));
    body.appendChild(r6);

    var r7 = mkRow();
    var fw = mkEl("div",{"class":"fileWrap"});
    fw.appendChild(mkEl("div",{"class":"fileBtn","id":"bm-lib-filebtn"},"Choose image"));
    fw.appendChild(mkEl("div",{"class":"fileName","id":"bm-lib-filename"},"No file selected"));
    r7.appendChild(fw);
    r7.appendChild(mkEl("input",{"id":"bm-lib-file","type":"file","accept":"image/*"}));
    body.appendChild(r7);

    var r8 = mkRow();
    r8.appendChild(mkEl("button",{"class":"action","id":"bm-lib-add","type":"button"},"Add new template"));
    r8.appendChild(mkEl("button",{"class":"action","id":"bm-lib-edit","type":"button","title":"Edit selected template"},"Edit selected"));
    body.appendChild(r8);

    var r9 = mkRow();
    r9.appendChild(mkEl("button",{"class":"action","id":"bm-lib-exportcurrent","type":"button"},"Export current"));
    r9.appendChild(mkEl("button",{"class":"action","id":"bm-lib-exportall","type":"button"},"Export all"));
    r9.appendChild(mkEl("button",{"class":"action","id":"bm-lib-import","type":"button"},"Import"));
    r9.appendChild(mkEl("input",{"id":"bm-lib-importfile","type":"file","accept":"application/json,.json"}));
    body.appendChild(r9);

    var r10 = mkRow(); r10.style.cssText = "margin:0;color:gray;";
    r10.appendChild(mkEl("span",{},"v" + SCRIPT_VERSION));
    r10.appendChild(mkEl("span",{"style":"width:100%;text-align:right;"},"with love, from jaz"));
    body.appendChild(r10);
    ...
    icon.id = "bm-lib-icon";
    var iconBtn = mkEl("button",{"type":"button","title":"Open Template Library"},"[Lib]");
    icon.appendChild(iconBtn);
  ...
  GM_registerMenuCommand("TLL: Reset panel position", function() {
    saveUI({"minimized":false,"panel":{"right":10,"top":110},"icon":{"right":10,"top":110}});
    var p = qs("#bm-lib"), ic = qs("#bm-lib-icon");
    if (p) p.remove(); if (ic) ic.remove();
    renderLibraryUI();
    alert("Panel reset to top-right corner!");
  });

  GM_registerMenuCommand("TLL: Show panel (un-minimize)", function() {
    var cur = loadUI(); cur.minimized = false; saveUI(cur);
    var p = qs("#bm-lib"), ic = qs("#bm-lib-icon");
    if (p && ic) { p.style.display = ""; ic.style.display = "none"; }
    else { saveUI({"minimized":false,"panel":{"right":10,"top":110},"icon":{"right":10,"top":110}}); renderLibraryUI(); }
  });

  GM_registerMenuCommand("TLL: Hard reset position", function() {
    if (!confirm("Reset panel position? Your templates will NOT be deleted.")) return;
    saveUI({"minimized":false,"panel":{"right":10,"top":110},"icon":{"right":10,"top":110}});
    var p = qs("#bm-lib"), ic = qs("#bm-lib-icon");
    if (p) p.remove(); if (ic) ic.remove();
    renderLibraryUI();
    alert("Position reset to default top-right!");
  });

  // Boot - show panel as soon as page is ready, no dependency on BM loading
  function tryBoot() {
    if (qs("#bm-lib") || qs("#bm-lib-icon")) return;
    try { renderLibraryUI(); }
    catch(err) { console.error("[TLL] renderLibraryUI failed:", err); }
  }
